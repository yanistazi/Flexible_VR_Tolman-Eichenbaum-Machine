...
def get_walking_data(start_state, adj, tran, prev_d, shiny_states, n_walk, params):
    ...
    for b in range(b_s):
        ...
        if params['world_type'] in ['square', 'rectangle']:
            ...
            pos[b, :], d[b, :, :], prev_d[b] = walk_family_tree(adj[env], tran[env], n_walk, start_state[b], prev_d[b],params)
...
def family_relation_type(s1, s2):
    ...
    rels = ['parent', 'child 1', 'child 2', 'sibling', 'grand parent']
...
def walk_family_tree(adj, tran, time_steps, start_state, prev_dir,params):
    position = np.zeros((1, time_steps + 1), dtype=np.int16)
    direc = np.zeros((params['n_actions'], time_steps))
